...
int main()
{
	...
	while (1)
	{
		...
		else if (choice == 2)
		{
			...
			for (int i = 0; i < numberOfCylinders; i++)
			{
				calculateDistanceVector(distanceVector, lcylinder, initialHeadPointerSSTF, numberOfCylinders);
				minIndex = findMinimum(distanceVector, numberOfCylinders);
				//printf("\t%d", lcylinder[minIndex]);
		...
		else if (choice == 3)
		{
			...
			if (headDirection < 0)
			{
				...
				overhead += lcylinder[numberOfCylinders - 1]-0;
